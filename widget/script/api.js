...
(function(window){
     ...
     * @date: 2018-11-13
     * @desc: 系统通用数据ajax调用方法
     * @param: sRequest JSON对象 接口参数规定的入参
     * @param：fnSuc 成功回调函数(可选参数)
     */
    u.ajaxValueJQ = function(sRequest, fnSuc) {
        var json = {};
        var data = {};
        var values = {};
        sRequest.apiKey = 'JQ';
        sRequest.token = 'nb';
        sRequest.timeStamp = new Date().getTime();
        values.sRequest = sRequest;
        data.values = values;
        json.data = data
        json.url = $api.getStorage('serverIp');
        json.dataType = 'json';
        json.method = 'post';
        json.timeout = 10;
        api.ajax(json,
     ...
     * @param：fnSuc 成功回调函数(可选参数)
     */
    u.ajaxJQ = function(sRequest, fnSuc) {
        var json = {};
        var data = {};
        var values = {};
        sRequest.apiKey = 'JQ';
