    ...
    auiPopup.prototype = {
        ...
        show: function(el){
	        ...
	        document.querySelector(__MASK).addEventListener("touchstart", function(event){
	        	...
	        	self.hide(el);
                if (el.className.indexOf('aui-popup-show') > -1) {
                    var winName = api.pageParam.winName;
                    var frameName = api.pageParam.frameName;
                    var funcName = api.pageParam.funcName;
                    if (winName && frameName && funcName) {
                        api.execScript({
                            winName: winName,
                            frameName: frameName,
                            script: funcName
                        });
                    } else if (frameName && funcName) {
                        api.execScript({
                            frameName: frameName,
                            script: funcName
                        });
                    } else if (winName && funcName) {
                        api.execScript({
                            winName: winName,
                            script: funcName
                        });
